...
class FinalQuery {
  ...
  #getMapper(type) {
    ...
    if (type instanceof UnionType) {
      const subTypes = type.types.map(getTypeInstance);
      const subMappers = subTypes.map((subType) => this.#getMapper(subType));
      if (subMappers.length > 0) {
        const firstMapper = subMappers[0];
        if (subMappers.every((mapper) => mapper === firstMapper)) {
          return firstMapper;
        }
      }
    }
...
class FromQuery {
   ...
   * @template {AnyObjectShape} U
   * @overload
   * @param {(...args: FctArgs<T>) => ArrayPick<U>} fromFct
   ...
   * @overload
   * @param {(...args: FctArgs<T>) => ArrayPick<U>} fromFct
   * @param {true} withKeyValue
   * @returns {FromQuery<[...T, {key: NumberType, value: ObjectType<GetShape<U>>}]>}
   */
  /**
   * @template {AnyObjectShape} U
   * @param {(...args: FctArgs<T>) => ArrayPick<U>} fromFct
   * @param {boolean} [withKeyValue]
   * @returns {FromQuery<[...T, GetShape<U>]> | FromQuery<[...T, {key: NumberType, value: ObjectType<GetShape<U>>}]>}
   */
  fromEach(fromFct, withKeyValue) {
    const from = this.#queryParts.from;
    const currentFrom = Array.isArray(from) ? from : [from];
    const eachAlias = `_each_${currentFrom.length}`;

    const arrayPick = fromFct(...this.#picks, pickOperators);

    if (withKeyValue) {
      const pick = new RowPick(
        object({
          key: number,
          value: toObjectType(arrayPick.type.valuesType),
        }),
        {alias: eachAlias}
      );

      /** @type {[...WrapRowPick<T>, RowPick<{key: NumberType, value: ObjectType<GetShape<U>>}>]} */
      const newPicks = [...this.#picks, pick];

      const newFrom = {
        table: pickOperators.each(arrayPick.expression),
        alias: eachAlias,
      };

      const queryParts = {
        ...this.#queryParts,
        from: [...currentFrom, newFrom],
      };

      return new /** @type {typeof FromQuery<[...T, {key: NumberType, value: ObjectType<GetShape<U>>}]>} */ (FromQuery)(
        this.#getTableSchema,
        newPicks,
        queryParts,
        this.#database
      ...
      const pick = new RowPick(toObjectType(arrayPick.type.valuesType), {
        expression: pickOperators.eachValue(eachAlias),
      });

      /** @type {[...WrapRowPick<T>, RowPick<GetShape<U>>]} */
      const newPicks = [...this.#picks, pick];
      ...
      return new /** @type {typeof FromQuery<[...T, GetShape<U>]>} */ (FromQuery)(
        this.#getTableSchema,
        newPicks,
        queryParts,
        this.#database
