 ...
 * @returns {string}
 */
function getTableName(queryParts) {
  if (typeof queryParts.from === 'string') {
    return queryParts.from;
  }
  return queryParts.from.alias;
}
...
class SelectQuery extends FinalQuery {
  ...
  where(fct) {
    const queryParts = {
      ...
      where: mergeWhere(
        ...
        fct(rowPick(this.#type, getTableName(this.#queryParts)), operators)
      ),
    };
    return new SelectQuery(this.#database, this.#type, queryParts);
  }
  ...
  orderBy(fct) {
    const queryParts = {
      ...this.#queryParts,
      orderBy: fct(rowPick(this.#type, getTableName(this.#queryParts)), operators),
    };
    return new SelectQuery(this.#database, this.#type, queryParts);
  }
  ...
  groupBy(fct) {
    const queryParts = {
      ...this.#queryParts,
      groupBy: fct(rowPick(this.#type, getTableName(this.#queryParts)), operators),
    };
    return new SelectQuery(this.#database, this.#type, queryParts);
  }
...
class FromQuery {
   ...
   */
  scope(fct) {
    const value = fct(rowPick(this.#type, getTableName(this.#queryParts)));
    const queryParts = {
      ...this.#queryParts,
      scope: value,
    };
   ...
   * @template {keyof T} F
   * @param {F} field
   * @returns {SelectQuery<T, t<T[F]>>}
   */
  field(field) {
    const row = rowPick(this.#type, getTableName(this.#queryParts));
    const queryParts = {
      ...this.#queryParts,
  ...
  select(fct) {
    const queryParts = {
      ...this.#queryParts,
      select: fct(rowPick(this.#type, getTableName(this.#queryParts)), operators),
    };
    return new SelectQuery(this.#database, this.#type, queryParts);
  }
