...
/**
 * @typedef {AnyPick | Operators["as"]} SelectValue
 */
 ...
 *   select: SelectValue[] | Record<string, SelectValue> | SelectValue,
  ...
  fields(values) {
    ...
    const queryParts = {
      ...
      select: values.map((value) =>
        scope.get(value).as(/** @type {string} */ (value))
      ),
    };
    return new ScopedSelectQuery(this.#database, this.#type, queryParts);
  }
...
class FromQuery {
  ...
  fields(fields) {
    ...
    const queryParts = {
      ...this.#queryParts,
      select: fields.map((fieldName) =>
        row.field(fieldName).as(/** @type {string} */ (fieldName))
      ),
    };
    return new SelectQuery(this.#database, this.#type, queryParts);
  }
