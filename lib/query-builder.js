...
/**
 * @typedef {{db?: string, dbId?: undefined, table: string, alias?: string} | {db?: undefined, dbId?: undefined, table: Subquery, alias?: string}} TableAndAlias
 */
...
class FinalQuery {
  ...
  json() {
    let attachs = [];
    if (this.#queryParts.attachs) {
      attachs = Array.from(this.#queryParts.attachs.entries()).reduce(
        (arr, [id, alias]) => [...arr, {id, alias}],
        attachs
      );
    }

    const ext = path.extname(this.#database.name);
    const db = path.basename(this.#database.name, ext);
    const {sql, values} = queryToSql(this.#queryParts);
    return {db, sql, values, attachs};
  }
...
class FromQuery {
  ...
  _join(joinOperator, tableName, shape, joinFct, mergePicks) {
    ...
    if (queryParts.attachs && tableSchema.db && tableSchema.dbId) {
      queryParts.attachs.set(tableSchema.dbId, tableSchema.db);
    }
...
class QueryBuilder {
  ...
  from(tableName, shape) {
    ...
    /** @type {Map<string, string>} */ const attachs = new Map();
    const queryParts = {
      from,
      withs: this.#withs,
      attachs,
