...
const {optionalObjectShape} = require('./optional-object-type.js');
...
/**
 * @typedef {BasePick | Aggregator | MathOperator | Operators['if']} SelectValue
 */
/**
 * @template T
 ...
 * @param {T} shape
 * @returns {{pick: RowPick<GetShape<T>>, condition: BooleanValue | null}}
 */
function useTableSchema(tableSchema, shape) {
  ...
  if (tableSchema.scope && tableSchema.baseShape) {
    ...
    if (tableSchema.scopeCondition) {
      condition = tableSchema.scopeCondition(basePick, operators);
    }
...
class FromQuery {
  ...
  _join(joinOperator, tableName, shape, joinFct, mergePicks) {
    ...
    const joinCondition = joinFct(...this.#picks, pick, operators);
    const queryParts = {
      ...this.#queryParts,
      joins: mergeJoin(this.#queryParts.joins, {
        ...
        constraint: condition
          ? operators.and(condition, joinCondition)
          : joinCondition,
      }),
    };
   ...
   * @template {AnyObjectShape} U
   * @template {[...ObjectShape[], ObjectShape]} V
   * @typedef {(tableName: string, shape: U, joinFct: (...args: FctArgs<AllShapes<T,U>>) => BooleanValue) => FromQuery<V>} JoinFunction
   */
...
class QueryBuilder {
  ...
  from(tableName, shape) {
    ...
    const {pick, condition} = useTableSchema(tableSchema, shape);
    if (condition) {
      queryParts.where = mergeWhere(queryParts.where, condition);
    }
    return new FromQuery(
      this.#getTableSchema,
      [pick],
