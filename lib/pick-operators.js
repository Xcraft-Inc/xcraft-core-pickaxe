...
const pickOperators = {
  ...
  /**
   * @template {Record<string,PickOrValue>} T
   * @param {T} fields
   * @returns {ObjectPick<{[K in keyof T]: PickOrValueType<T[K]>}>}
   */
  object(fields) {
    const shape = /** @type {{[K in keyof T]: PickOrValueType<T[K]>}} */ (Object.fromEntries(
      Object.entries(fields).map(([name, pickOrValue]) => [
        name,
        getPickOrValueType(pickOrValue),
      ])
    ));
    return new ObjectPick(object(shape), $o.object(fields));
  },
};
