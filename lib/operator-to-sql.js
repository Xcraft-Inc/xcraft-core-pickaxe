...
const operators = {
  ...
  field({tableName, field, useTableName}, context) {
    // Note: field is not validated
    if (typeof field === 'string') {
      if ((context.useTableNames || useTableName) && tableName) {
  ...
  eachValue({name}, context) {
    if (name) {
      if (!name.match(/^_each_\d+$/)) {
        throw new Error(`Unsupported name for json_each: '${name}'`);
      }
      return `${name}.value`;
    }
  ...
  object({fields}, context) {
    const entries = Object.entries(fields).map(([name, field]) => {
      if (typeof name !== 'string') {
        throw new Error('Bad object field name');
      }
      return [escape(name), sql(field, context)];
    });
    return `json_object(${entries.flat().join(',')})`;
  },
};
