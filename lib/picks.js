...
class ObjectPick {
  ...
  get keysType() {
    const keys = /** @type {(keyof T)[]} */ (Object.keys(
      this.#type.properties
    ));
    return new EnumerationType(keys);
  }

  get valuesType() {
    const values = Object.values(this.#type.properties);
    const values2 = /** @type {Type<T[keyof T]>[]} */ (values.map(
      getTypeInstance
    ));
    return new UnionType(values2);
  }
  ...
  /**
   * @param {T[keyof T] | ValuePick<T[keyof T]>} value
   */
  includes(value) {
  ...
  /**
   * @param {(value: PickOf<T[keyof T]>, key: ValuePick<keyof T>) => Operator} func
   */
  some(func) {
    return $o.some(
      this.value,
      func(
        makeTypePick(this.valuesType, {
          field: $o.eachValue(),
          path: [],
        }),
        new ValuePick(this.keysType, {
          field: $o.eachKey(),
          path: [],
        })
      )
  ...
  /**
   * @returns {ArrayPick<keyof T>}
   */
  keys() {
    return new ArrayPick(array(this.keysType), {
  ...
  /**
   * @returns {ArrayPick<T[keyof T]>}
   */
  values() {
    return new ArrayPick(array(this.valuesType), {
